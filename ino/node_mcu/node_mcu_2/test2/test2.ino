...
// Pin mapping notes for node2 (NodeMCU D# naming -> ESP8266 GPIO):
//  pump            -> D6 (GPIO12)
//  servo5          -> D1 (GPIO5)
//  servo6          -> D2 (GPIO4)
//  step_STEP       -> D7 (GPIO13)
...
void onMessage(char* topic, byte* payload, unsigned int len) {
  ...
  } else if (target_room == 2 || target_room == 3) {
    Serial.println("DIR,R");
  }

  // trigger room-specific actuators
  if (target_room == 2) {
    Serial.println("SERVO5,1");
  } else if (target_room == 3) {
    Serial.println("SERVO6,1");
...
void processSerialLine(const String &line) {
  ...
  if (s.equalsIgnoreCase("done")) {
    if (activeQueue >= 0) {
      Serial.printf("[node2] Arduino done for queue %d\n", activeQueue);
...
void setup() {
  Serial.begin(9600);
  delay(500);
  Serial.println("NodeMCU MQTT->Serial forwarder ready! (node 2)");
...
void loop() {
  ...
  if (activeQueue >= 0 && cmdSentTime > 0 && millis() - cmdSentTime > CMD_TIMEOUT_MS) {
    Serial.printf("[node2] Arduino timeout for queue %d after %d ms\n", activeQueue, CMD_TIMEOUT_MS);
